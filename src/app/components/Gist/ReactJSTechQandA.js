...
const ReactJSTechQandA = () => {
  ...
  const fetchQandAData = async () => {
    setIsLoading(true);
    try {
      const reactjsTechData = await getTechQandA(
        "react",
        currentPage,
        ITEMS_PER_PAGE
      );
      setqueAnsData(reactjsTechData.storyData);
      settotalQandA(reactjsTechData.totalData);
      setIsLoading(false);
    } catch (error) {
      console.error(error);
      setIsLoading(false);
        ...
        <div className="h-full min-h-[400px] md:max-h-[600px] max-h-full">
          <div className="container max-w-[1280px] main-section-padding !pt-24 mx-auto">
            <div className="flex flex-col items-start justify-center h-full min-h-[300px] md:max-h-[600px]  sxl:mt-20 mt-10 max-h-full">
              <Heading type="h1" className="text-white" text="ReactJS Q & A" />
