...
const ProjectSolutionSection = ({ projectSolutions }) => {
  return (
    <section className="main-section-padding" id="solution">
...
What We Built
        ...
        {projectSolutions?.ListItem?.length > 0 && (
          <ul className="max-w-[800px] list-none">
            {projectSolutions.ListItem.map(({ Title, Display_text }, index) =>
              Display_text ? (
                <li
                  key={index}
                  className="flex gap-4 items-start py-5 border-b border-gray-200 last:border-b-0"
                >
                  <div className="w-9 h-9 rounded-full bg-[#ecfdf5] text-[#059669] flex-shrink-0 flex items-center justify-center text-sm font-bold">
                    &#10003;
                  </div>
                  <div>
                    {Title && (
