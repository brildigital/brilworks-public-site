  ...
  return (
    <section className="py-20 relative overflow-hidden" ref={ref}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
          {outcomes.map((outcome, index) => (
            <motion.div
              ...
              className="bg-white backdrop-blur-sm p-4 rounded-xl border border-[#e4eaf1] hover:shadow-2xl hover:shadow-[#2f6bff1A] hover:border-primary/40 transition-all duration-500 group"
              data-testid={`card-outcome-${index}`}
            >
              {/* <div
                className={`w-12 h-12 ${outcome.bgColor} rounded-lg flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300 border border-[#e4eaf1]`}
              >
                <outcome.icon
                  className={`${outcome.iconColor} text-xl group-hover:text-[#1e4fd6] transition-colors duration-300`}
                />
              </div> */}
        ...
        >
          <div
            className="inline-flex items-center text-xs md:text-base space-x-2 bg-[#eaf1ff] text-themeColor px-4 py-2 rounded-full font-medium"
